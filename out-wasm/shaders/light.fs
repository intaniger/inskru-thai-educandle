...
float sceneDist(vec2 p)
{
	...
	float b1 =  boxDist(		translate(p, vec2(41, 41)), vec2(40, 40), 	0.0);
...
float shadow(vec2 p, vec2 pos, float radius)
{
	...
	{				
        ...
        float sd = circleDist(translate(p + dir * dt, vec2(300, 100)), 	30.0);
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    vec2 light2Pos = vec2(100, 100);
	vec4 light2Col = vec4(0.75, 1.0, 0.5, 1.0);
	setLuminance(light1Col, 0.5);
	
	
	
	// gradient
	vec4 col = vec4(0,0,0, 1.0) * (1.0 - length(c - p)/iResolution.x);
	// grid
	...
	// light
	col += drawLight(p, light1Pos, light1Col, dist, 100.0, 5.0);
    // col += drawLight(p, light2Pos, light2Col, dist, 300.0, 10.0);
	// shape fill
